...
//  MetaMerge
//
//  Created by Dan Hushon on 11/3/17.
...
protocol EncodingOptions {
    static var version: Int {get set}
}

struct TensorBoxRect: Codable , EncodingOptions{
...
public struct TensorBoxElement: Codable, EncodingOptions {
    static var version: Int = 1
...
public class TensorBoxParser: VOCParser {
        ...
        if url.isFileURL {
            do {
                ...
                do {
                    ...
                    print("Maybe not a v1 file trying v2? \(error)")
                ...
                do {
                    ...
                    print("Couldn't decode file using v1 or v2: \(error)")
    ...
    // this is bit of a hack and will not support parallelization safely if different encoders require different versions
    func encodeOption(version: Int) {
        if (self.version != version) {
            self.version = version
            TensorBoxElementSet.version = self.version
            TensorBoxElement.version = self.version
            TensorBoxRect.version = self.version
        }
    }
    ...
    func encode(url: URL, tbes: TensorBoxElementSet?) throws {
        ...
        do {
            ...
            encoder.outputFormatting = .sortedKeys // follow the lexicographical order
